...
-   **Goals**: This is when a player kicks the ball between the two center goal post and it is NOT touched by another player or touches either goalpost.
    ...
    ![gif_test](https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNTlmbmRtMW5ydWhldjRsc2NzMTJsNGxqYmVlNzVxMjk2aG50ZmR3MyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/55pSPZpaAQqtMCNF0E/giphy.gif)
...
afl_position_stats<-afl_stats%>%
  ...
  mutate(games = n_distinct(providerId),
         ...
         behinds_per_game = round(behinds/games,1),
         disposals_per_game = round(disposals/games,1),
         handballs_per_game = round(handballs/games,1),
         kicks_per_game = round(kicks/games,1),
         tackles_per_game = round(tackles/games,1),
         marks_per_game = round(marks/games,1)
...
# Define AFL custom color palette for position types
afl_colors <- c(
    Defender = "#778899",
    Midfield = "#0288D1",
    Forward = "#FFA000"
...
# Map the custom palette to the 'Position.Type' factor levels using scales::col_factor()
position_type_colors <- scales::col_factor(palette = afl_colors, domain = c("Defender", "Midfield", "Forward"))

# Define a custom blue color gradient function with the specified 8 colors
custom_blue_palette <- c("#f7fbff", "#deebf7", "#c6dbef", "#9ecae1", 
                         "#6baed6", "#4292c6", "#2171b5", "#084594")
blue_gradient <- scales::col_numeric(pal = custom_blue_palette, domain = NULL)

# Correctly adjust 'Position Type' to a factor with levels in the exact order
# Make sure the levels exactly match the data in your dataframe
afl_pos_stats$Position.Type <- factor(afl_pos_stats$Position.Type, 
                                        levels = c("Forward", "Midfield", "Defender"))

# Generate the GT table, now sorting by 'Position Type' in the specified order
afl_table <- afl_pos_stats %>%
  arrange(Position.Type) %>%
  select(Position.Name, Position.Type, position, goals_per_game, behinds_per_game, disposals_per_game, handballs_per_game, kicks_per_game, tackles_per_game, marks_per_game) %>%
  gt() %>%
  tab_header(
  ...
  cols_label(
    ...
    behinds_per_game = "Behinds Per Game",
    disposals_per_game = "Disposals Per Game",
    handballs_per_game = "Handballs Per Game",
    kicks_per_game = "Kicks Per Game",
    tackles_per_game = "Tackles Per Game",
    marks_per_game = "Marks Per Game"
  ) %>%
  tab_style(
  ...
  data_color(
    columns = vars(Position.Type),
    colors = position_type_colors
  ) %>%
  data_color(
    columns = vars(goals_per_game, behinds_per_game, disposals_per_game, handballs_per_game, kicks_per_game, tackles_per_game, marks_per_game),
